...
class _QuestionItemListState extends State<QuestionItemList> {
  ...
  void _loadCSV() async {
    final rawData = await rootBundle.loadString("assets/data/quiz.csv");
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      ...
      List<dynamic> questions = _data.map((row) => row[0]).toList();
...
class DataService {
  Future<List<List>> getCsvDataFromLocal() async {
    final csvFile = File('assets/data/quiz.csv').openRead();
